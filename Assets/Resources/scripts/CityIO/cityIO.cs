...
public class cityIO : MonoBehaviour
{
    ...
    private string urlStart = "https://cityio.media.mit.edu/api/table/citymatrix_";
    ...
    private void addLeantoCubes()
    {
        for (int i = 0; i < gridObjects.Count; i++)
        {
            var p = gridObjects[i].gameObject.transform.position;
            GameObject control = GameObject.Instantiate(_ball,
            new Vector3(p.x, p.y * 2, p.z),
            transform.rotation) as GameObject; //spwan prefab text
